---
description: Frontend engineering review — walks the 7 Matt Pocock forcing questions (device, LCP target, rendering, bundle budget, SEO vs auth, design system, WCAG), picks the framework + rendering profile, forks into specialists (a11y-audit, performance-profiler, epic-design). Invokes the ds-frontend-engineer agent with context fork.
argument-hint: "<problem or surface to review>"
---

# /ds:frontend-review — Frontend engineering review

Use the `ds-frontend-engineer` agent (uses `context: fork`) to handle this inquiry:
...
- `/ds:fullstack-review` — full-stack lens (parent)
- `/ds:backend-review` — for API contract on the consumer side
- `/ds:engineer-grill` — cross-role 21-question grill
