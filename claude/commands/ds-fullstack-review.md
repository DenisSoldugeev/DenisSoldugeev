---
description: Fullstack engineering review — walks the 7 Matt Pocock forcing questions, picks the profile, forks into POWERFUL specialists (api-design-reviewer, database-designer, slo-architect). Invokes the ds-fullstack-engineer agent with context fork.
argument-hint: "<problem or codebase to review>"
---

# /ds:fullstack-review — Fullstack engineering review

Use the `ds-fullstack-engineer` agent (which uses `context: fork` to keep the parent thread clean) to handle this inquiry:
...
5. **Fork into specialists** (one at a time, depth-first):
   ...
   - `ds-karpathy-reviewer` before any commit
...
- `/ds:frontend-review` — frontend-only deep dive
- `/ds:backend-review` — backend-only deep dive
- `/ds:engineer-grill` — cross-role 21-question forcing-question runner
