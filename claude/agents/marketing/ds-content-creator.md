...
The ds-content-creator agent is the marketing domain's **content execution specialist**. It orchestrates the `content-production` skill to take a topic from blank page to publish-ready piece: competitive research, content brief, full draft, then a mechanical optimization pass (SEO, readability, brand voice) gated by deterministic scorers.
...
- **vs `ds-aeo`**: ds-aeo optimizes finished content for LLM citation (AEO). This agent produces the content; run ds-aeo afterwards when AI-search citation matters.
...
- `../../marketing-skill/skills/content-production/references/ai-citation-readiness.md` — AEO-adjacent readiness checks (pair with ds-aeo)
...
- Optimizing for ChatGPT/Perplexity citation → hand off to [ds-aeo](ds-aeo.md).
...
- [ds-aeo](ds-aeo.md) — optimizes this agent's output for LLM citation (run after production)
- [ds-demand-gen-specialist](ds-demand-gen-specialist.md) — uses this agent's content as demand-gen fuel (gated assets, nurture content)
- [ds-webinar-marketer](ds-webinar-marketer.md) — webinar funnels that consume produced content
