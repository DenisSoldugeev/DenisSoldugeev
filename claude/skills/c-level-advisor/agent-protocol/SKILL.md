...
All C-suite skills and `/ds:*` commands read and write decisions in **one** place — the two-layer model owned by `/ds:decide` and the decision-logger skill:
...
- **Layer 2 (approved)** stores only founder-approved decisions. This is what board meetings, `/ds:office-hours`, and `/ds:founder-mode` load. Prevents hallucinated consensus.
- Writers: `/ds:decide` and the Chief of Staff (post board-meeting Phase 5). Individual role agents never write decisions directly.
