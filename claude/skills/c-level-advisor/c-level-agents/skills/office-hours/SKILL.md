---
name: "office-hours"
description: "/ds:office-hours <topic> — YC-style 6-question founder interrogation before any advice. Forces clarity on problem, customer, distribution, defensibility, capital, and founder fit. Use when a founder question is too vague to route — e.g. 'should we grow faster?' — or before drafting a strategy brief."
---

# /ds:office-hours — Six-Question Founder Interrogation

**Command:** `/ds:office-hours <topic>`
...
**Assessment** (one of):
- 🟢 GREEN — ship the brief to /ds:boardroom
...
- Single-role question → corresponding `/ds:{role}-review`
- Multi-role question → `/ds:brief` then `/ds:boardroom`
...
- `/ds:brief` — turn the answers into a one-page strategy brief
- `/ds:boardroom` — multi-role deliberation
- `/ds:founder-mode` — let the system pick the next step

## Related Agents

- All ds-* advisors consume the brief output
- `ds-chief-of-staff` triggers `/ds:office-hours` when intake is unclear
