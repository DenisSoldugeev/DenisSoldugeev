---
name: "cross-eval"
description: "/ds:cross-eval <memo> — Multi-model consensus on a board memo or strategy brief. Claude + Codex + Gemini cross-review with graceful degradation. Use when a high-stakes memo needs an independent sanity check before the boardroom — e.g. a bet-the-company pivot or fundraise terms."
---

# /ds:cross-eval — Multi-Model Consensus

**Command:** `/ds:cross-eval <memo-or-brief>`
...
- `/ds:decide` — if consensus is GO
- `/ds:freeze` — if consensus is PAUSE
- `/ds:boardroom` (re-run) — if consensus is STOP
