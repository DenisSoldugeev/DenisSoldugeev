---
name: "onboard"
description: "/ds:onboard — Founder interview that populates ~/.claude/company-context.md using the canonical 7-dimension ds-onboard schema. The first command to run when starting with c-level-agents. Use when setting up the virtual C-suite for a new company, or when advisors lack company context — e.g. before a first /ds:boardroom or after a fundraise changes the numbers."
---

# /ds:onboard — Founder Interview

**Command:** `/ds:onboard`

The first command to run when adopting c-level-agents. A structured founder interview that produces `~/.claude/company-context.md` — the file every ds-* advisor reads before responding. Without this, the advisors are guessing.

## What This Produces

`~/.claude/company-context.md` — a single file with the durable facts about the company. Read by:
- `ds-chief-of-staff` (routing decisions)
- Every ds-* advisor (context for any question)
- `/ds:brief` (assumptions in any new decision)
...
- Always before a `/ds:boardroom` for a high-stakes decision
...
- Skill: [`ds-onboard`](../../../ds-onboard/SKILL.md) — the underlying interview protocol
- Skill: [`context-engine`](../../../context-engine/SKILL.md) — reads this file
