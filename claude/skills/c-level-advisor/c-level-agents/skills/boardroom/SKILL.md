---
name: "boardroom"
description: "/ds:boardroom <brief> — 6-phase multi-role deliberation across the C-suite with Phase 2 isolation, critic pre-screen, and synthesis. Outputs a board memo. Use when a decision spans multiple executive domains — e.g. a pricing change touching finance, positioning, and product, or a raise-vs-cut runway call."
---

# /ds:boardroom — Multi-Role Boardroom Deliberation

**Command:** `/ds:boardroom <brief-path>`
...
- Each advisor critiques the others' positions on the dimensions they own:
  - ds-cfo-advisor critiques the math
  - ds-ciso-advisor critiques the risk
  - ds-cpo-advisor critiques the JTBD
  - ds-cmo-advisor critiques the positioning
  - ds-cro-advisor critiques the revenue math
  - etc.
...
- Approved memo routes to `/ds:decide` for logging.
...
|---|---|---|
| ds-ceo-advisor | A | <reason> |
| ds-cfo-advisor | A | <reason> |
| ds-cto-advisor | B | <reason> |
| ... | | |
...
- `/ds:cross-eval` → multi-model sanity check (optional, high-stakes)
- `/ds:freeze N` → cooldown lock (optional, irreversible)
```
...
3. Invoke each ds-* advisor independently (Phase 2)
4. Collect positions
...
- `/ds:decide` — log approved memo
- `/ds:cross-eval` — high-stakes second opinion
- `/ds:freeze` — cooldown lock

## Related

- Agent: [`ds-chief-of-staff`](../../agents/ds-chief-of-staff.md)
- Skills: [`board-meeting`](../../../board-meeting/SKILL.md), [`executive-mentor`](../../../executive-mentor/)
