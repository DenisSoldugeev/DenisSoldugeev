...
**Command:** `/ds:post-mortem <decision-path>`
...
- At the 90-day checkpoint (auto-scheduled by `/ds:decide`)
...
- The decision record (output of `/ds:decide`)
- The execution plan (output of `/ds:execute`)
...
- LOSS → schedule follow-up boardroom: `/ds:brief` for the next call
```

## Why Pre-Committed Criteria Matter

The biggest temptation in post-mortems is retroactive justification: "we always knew X, that's why we did Y." Pre-committed criteria, signed at `/ds:decide` time, eliminate that move. The numbers either matched or they didn't.

## Why Revisit Dissent

The dissent column from `/ds:boardroom` is the single most useful piece of organizational memory. Most of the time, the dissenter was directionally right. Revisiting and scoring it builds calibration over years.

## Routing

- `/ds:brief` — if the post-mortem surfaces a new decision
- `/ds:freeze` — if the post-mortem reveals a process gap that needs cooldown enforcement
- Updates to company-context.md via `ds-onboard`
