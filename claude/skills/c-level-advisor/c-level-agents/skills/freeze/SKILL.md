---
name: "freeze"
description: "/ds:freeze <decision> <days> — Lock a strategic decision for a cooldown period to prevent impulse reversal. Mirrors gstack's safety primitives for the business layer."
---

# /ds:freeze — Cooldown Lock on a Decision

**Command:** `/ds:freeze <decision-path> <days>`
...
5. ds-chief-of-staff router now refuses to re-route this topic to the boardroom until:
...
**Override condition:** Kill criterion <name> triggers OR founder issues `/ds:unfreeze` with stated reason
```
...
|---|---|---|
| <decision title> | YYYY-MM-DD | <kill criterion or /ds:unfreeze> |
```
...
If a kill criterion in the decision triggers, the freeze auto-releases and the chief-of-staff routes immediately to `/ds:post-mortem`. The freeze does not protect against reality; it protects against impulse.
...
- `/ds:unfreeze` — explicit early release
- `/ds:post-mortem` — auto-triggered if kill criterion fires
- `/ds:boardroom` — blocked until unfreeze or expiry

## Related

- Skill: [`decision-logger`](../../../decision-logger/SKILL.md)
- Agent: [`ds-chief-of-staff`](../../agents/ds-chief-of-staff.md) — enforces freezes in routing
