...
The ds-ciso-advisor orchestrates the `ciso-advisor` skill to make security a first-class executive concern, not a checkbox. Forces founders to define threat models, blast radii, and IR runbooks before any production decision involving customer data.

Pairs with `ds-cto-advisor` (security architecture), `ds-cfo-advisor` (risk quantification → insurance + audit cost), and the ra-qm-team domain (ISO 27001, SOC 2, GDPR). Reports critical risks to `ds-ceo-advisor` immediately.
