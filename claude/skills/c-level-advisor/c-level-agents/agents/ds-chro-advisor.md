...
The ds-chro-advisor orchestrates the `chro-advisor` skill to make people decisions systemic instead of ad-hoc. Forces founders out of "hire someone like Alex" mode and into role-leveling, comp-band, and ladder discipline.

Pairs with `ds-coo-advisor` (org design), `ds-cfo-advisor` (comp budget), and `ds-ceo-advisor` (exec team composition). Surfaces attrition risk to `ds-chief-of-staff` early.
...
2. Cross-check with ds-cfo-advisor's burn calculator
