...
The ds-cco-advisor orchestrates the `chief-customer-officer-advisor` skill across the four decisions a startup CCO actually faces:
...
Differentiates from:
- `ds-cro-advisor` (revenue math, expansion comp, ramp): CRO owns revenue *math*, CCO owns customer *experience*
- `ds-cmo-advisor` (positioning): CMO owns pre-sale; CCO owns post-sale
- `ds-cpo-advisor` (product strategy): CCO surfaces product gaps via churn taxonomy; CPO decides roadmap
...
# 5. Cross-check product gaps surfaced by churn with ds-cpo-advisor
...
# 5. Cross-check hiring plan + comp design with ds-chro-advisor
# 6. Output: 12-month hiring plan; log via /ds:decide
```
...
4. Cross-check with ds-chro-advisor on comp + leveling
5. Cross-check with ds-cro-advisor on whether the AM-vs-CSM split is needed
...
- [ds-cro-advisor](ds-cro-advisor.md) — Revenue math, NRR, expansion comp (CCO owns experience; CRO owns math; clean split)
- [ds-cpo-advisor](ds-cpo-advisor.md) — Product gaps surfaced by churn (CCO feeds; CPO decides)
