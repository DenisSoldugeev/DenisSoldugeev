...
The ds-cdo-advisor orchestrates the `chief-data-officer-advisor` skill across the four decisions a startup CDO actually faces:
...
Differentiates from `ds-cto-advisor` (architecture), `ds-ciso-advisor` (security/compliance), `ds-cpo-advisor` (product strategy), and `ds-general-counsel-advisor` (contract review). Each of those overlaps with one CDO concern but none owns the strategic data picture.
...
# 6. Log via /ds:decide; consider /ds:freeze 90 on multi-year SaaS contracts
```
...
- [ds-cfo-advisor](ds-cfo-advisor.md) — build-vs-buy TCO, M&A valuation math
- [ds-chro-advisor](ds-chro-advisor.md) — data team hiring, leveling, comp
