...
The ds-caio-advisor orchestrates the `chief-ai-officer-advisor` skill across the four decisions a startup CAIO actually faces:
...
Differentiates from `ds-cdo-advisor` (data strategy, training rights), `ds-cto-advisor` (architecture, scaling), `ds-ciso-advisor` (security, threat modeling), `ds-general-counsel-advisor` (contracts). Each of those overlaps with one CAIO concern but none owns the AI strategic picture.
...
# 5. Cross-check with ds-cdo-advisor if customer data is involved in fine-tune
# 6. Log via /ds:decide; consider /ds:freeze 60 on multi-year vendor commitment
```
...
# 5. Cross-check with ds-general-counsel-advisor on contract / liability implications
# 6. Cross-check with ds-ciso-advisor on technical safeguards
# 7. Log via /ds:decide
```
...
# 5. Cross-check with ds-cto-advisor on platform readiness + on-call capacity
# 6. Log via /ds:decide; pair with /ds:freeze if signing multi-year GPU commitment
```
...
5. Cross-check with ds-chro-advisor on comp + leveling
6. Cross-check with ds-cdo-advisor for AI/data team boundary
...
- [ds-chro-advisor](ds-chro-advisor.md) — AI team hiring + comp
