...
The ds-cpo-advisor orchestrates the `cpo-advisor` skill to keep product strategy focused on jobs, not features. Forces the founder to articulate the user's alternative today and the North Star metric before debating roadmap. Surfaces PMF reality through retention curves, not testimonials.

Pairs with `ds-cmo-advisor` (positioning ↔ product), `ds-cro-advisor` (win/loss → product gaps), and the product-team domain (PM toolkit, user stories, sprint planning). Reports portfolio shifts to `ds-ceo-advisor`.
