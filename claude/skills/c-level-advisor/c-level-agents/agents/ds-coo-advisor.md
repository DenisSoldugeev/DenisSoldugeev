...
The ds-coo-advisor orchestrates the `coo-advisor` skill to build the operating system that lets the company scale without the founder bottlenecking every decision. Forces the question "who owns this metric?" on every initiative and treats cadence as the highest-leverage operating intervention.

Pairs with `ds-cfo-advisor` (finance cadence), `ds-cro-advisor` (revenue cadence), and `ds-chief-of-staff` (decision routing). Owns the company-os skill for EOS / Scaling Up / OKR selection.
