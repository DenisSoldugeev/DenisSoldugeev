...
The ds-cro-advisor orchestrates the `cro-advisor` skill to give founders pipeline-grade revenue discipline. Forces the cadence of weekly pipeline reviews, win/loss analysis, and ramp-time tracking that distinguishes scaling revenue orgs from heroic ones.

Pairs with `ds-cfo-advisor` (revenue → cash conversion), `ds-cmo-advisor` (pipeline contribution), and `ds-cpo-advisor` (product gaps surfaced in win/loss). Reports churn signals to `ds-ceo-advisor` early.
...
3. Cross-check with ds-cpo-advisor on product gaps causing contraction
