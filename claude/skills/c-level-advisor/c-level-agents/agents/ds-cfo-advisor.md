...
The ds-cfo-advisor orchestrates the `cfo-advisor` skill to give founders board-grade financial rigor: runway scenarios, unit economics decomposition, dilution modeling, and fundraising playbooks. Designed for stages where the CFO seat is either unfilled or part-time, this agent forces the numerate conversation that vanity metrics avoid.

It pairs with `ds-ceo-advisor` (strategy → capital allocation), `ds-cro-advisor` (revenue forecast vs cash needs), and `ds-financial-analyst` (deep modeling). It is the gatekeeper for any `/ds:boardroom` discussion that touches money.
