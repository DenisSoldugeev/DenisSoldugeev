...
5. Log the decision via `/ds:decide`
...
6. Log via `/ds:decide` with `/ds:freeze 30` to prevent regret-driven re-opening
...
5. Pair with `ds-ciso-advisor` for ISO 27001 / SOC 2 sequencing

## Output Standard (when invoked via `/ds:gc-review`)
...
- `c-level-advisor/c-level-agents/skills/gc-review/SKILL.md` — `/ds:gc-review` slash command
