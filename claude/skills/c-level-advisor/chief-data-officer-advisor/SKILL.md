...
# 5. Cross-check with ds-general-counsel-advisor on top-3 mitigation items
# 6. Log via /ds:decide
```
...
# Cross-check with ds-cto-advisor on engineering capacity
# Cross-check with ds-cfo-advisor on 3-year TCO
# Log via /ds:decide; consider /ds:freeze 90 if signing a multi-year SaaS contract
```
...
4. Surface contractual carve-outs to ds-general-counsel-advisor for re-papering plan
...
4. Cross-check with ds-chro-advisor on comp bands and leveling
5. Identify the centralize-vs-embed trigger date

## Output Standards (when invoked via ds-cdo-advisor)
