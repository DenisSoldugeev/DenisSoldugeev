...
# 3. Cross-check with ds-cfo-advisor on budget commitment
# 4. Cross-check with ds-cto-advisor on engineering capacity (esp. for fine-tune)
# 5. Log via /ds:decide; consider /ds:freeze 60 on multi-year vendor commitment
```
...
# 4. Cross-check with ds-general-counsel-advisor on contractual implications
# 5. Cross-check with ds-ciso-advisor on technical safeguards
# 6. Log via /ds:decide
```
...
# 4. Cross-check with ds-cfo-advisor on capex commitment
# 5. Cross-check with ds-cto-advisor on platform readiness
# 6. Log via /ds:decide; pair with /ds:freeze if signing GPU commitment
```
