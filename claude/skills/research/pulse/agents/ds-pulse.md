...
- **vs ds-grill-master** (plan interrogator): different domain — pulse runs an *intake-then-search* workflow, grill walks a decision tree.
- **vs ds-grill-with-docs** (docs-anchored grill): different scope — pulse is about external sources, grill-with-docs is about internal CONTEXT.md.
- **vs ds-capture** (brain-dump organizer): different mode — pulse pulls external data, capture organizes user-provided dumps.
...
- Sibling command: [`/ds:pulse`](../commands/ds-pulse.md)
