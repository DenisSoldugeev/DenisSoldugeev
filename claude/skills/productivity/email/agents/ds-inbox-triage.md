...
**KB missing (halt):**
> "Knowledge base not found at `${WORKSPACE}/Email/`. Run `/ds:inbox-setup` first to build it. The triage skill needs at minimum `email-taxonomy.md` and `email-patterns.md` to operate."
...
The ds-inbox-triage agent orchestrates the `inbox-triage` skill across recurring inbox processing:
...
- **vs ds-inbox-setup** (companion): different mode — triage is fast-execution recurringly; setup is interview-driven once
- **vs ds-pulse** (research): different domain — triage is inbox-internal; pulse is external multi-source research
- **vs ds-capture** (brain-dump organizer): different artifact — triage processes inbox; capture organizes user-provided dumps
...
- [ds-inbox-setup](./ds-inbox-setup.md) — companion skill, writes the KB this skill reads
- [ds-pulse](../../../research/pulse/agents/ds-pulse.md) — external research (different domain)
- [ds-capture](../../../productivity/capture/agents/ds-capture.md) — brain-dump organizer (different mode)
...
- Sibling command: [`/ds:inbox-triage`](../commands/ds-inbox-triage.md)
