...
description: "/ds:inbox-triage — Recurring email triage execution. Reads 7-file KB built by /ds:inbox-setup. Classifies recent emails, drafts replies (NEVER SENDS), delivers report, updates KB. Run 1-3x/day or on demand. Halts with clear message if KB missing."
---

# /ds:inbox-triage — Recurring Email Triage

**Command:** `/ds:inbox-triage`

The `ds-inbox-triage` persona processes your inbox using the knowledge base built by `/ds:inbox-setup`. Designed for recurring runs (1-3x/day) with light intake — most invocations skip questions and run with KB-default preferences.
...
**Do NOT run if** the KB doesn't exist yet — the skill will halt and direct you to `/ds:inbox-setup` first.
...
## Trigger Phrases (auto-invoke without /ds:)
...
- KB files missing → halt; direct to `/ds:inbox-setup`
...
- Companion: [`/ds:inbox-setup`](./ds-inbox-setup.md) — must run first
- Agent: [`ds-inbox-triage`](../agents/ds-inbox-triage.md)
