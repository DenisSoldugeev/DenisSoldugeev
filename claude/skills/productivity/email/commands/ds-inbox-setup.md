---
name: "ds-inbox-setup"
description: "/ds:inbox-setup — Interactive 8-section interview that builds a personalized 7-file email-triage knowledge base. ~25-31 grill-me questions, one at a time. Run ONCE; re-run when business/pricing/priorities change. Companion to /ds:inbox-triage."
---

# /ds:inbox-setup — Email Triage Onboarding

**Command:** `/ds:inbox-setup`

The `ds-inbox-setup` persona walks an 8-section interview to build your personalized email triage knowledge base in `${WORKSPACE}/Email/`.
...
- `/ds:inbox-setup` (this command) — **writes** the KB (run once)
- `/ds:inbox-triage` — **reads + appends** the KB (run recurringly)
...
8. **Confirmation & Handoff** — file inventory + handoff message → directs you to `/ds:inbox-triage`
...
- Companion: [`/ds:inbox-triage`](./ds-inbox-triage.md) — runs after setup is complete
- Agent: [`ds-inbox-setup`](../agents/ds-inbox-setup.md)
