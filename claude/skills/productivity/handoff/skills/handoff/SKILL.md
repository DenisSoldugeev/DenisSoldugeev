...
On first invocation, the skill asks where to save handoffs so the project folder never gets cluttered. Setup is offered once via *"Run setup now? (Y/n)"* — answering N uses OS-temp defaults for this run and never re-prompts. The user can rerun setup any time via `/ds:handoff-setup`.
...
- `/ds:handoff [optional next-session description]` — generate the handoff.
- `/ds:handoff-setup` — reconfigure save location, retention, redaction.

## Agent

`ds-handoff-author` — Matt-voice persona orchestrating the skill. Terse, no-duplication, references-not-copies.
...
```
User: /ds:handoff "finish wiring the redaction linter and open a draft PR"
```
...
```
User: /ds:handoff "ship the migration"
...
| Step | Command |
|---|---|
| First-run setup | `/ds:handoff-setup` (or answer Y on first `/ds:handoff`) |
| Generate a handoff | `/ds:handoff [goal]` |
| Reconfigure later | `/ds:handoff-setup --reconfigure` |
| Project-specific config | `/ds:handoff-setup --project` |
