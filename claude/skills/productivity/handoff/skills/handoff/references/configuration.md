...
On first invocation of `/ds:handoff`, the skill detects whether a config exists. If none does, it asks:
...
Rerun setup any time with `/ds:handoff-setup` (re-prompts and writes config; removes the decline sentinel automatically).
...
Cleanup runs at the start of each `/ds:handoff` invocation. It is **mtime-guarded**: files with `mtime > ctime + 2s` (i.e., edited by the user after creation) are never deleted.
