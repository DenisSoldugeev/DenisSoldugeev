...
The ds-andreessen agent orchestrates the `andreessen` skill to:
...
- **vs ds-reflect** (productivity): reflect re-reads the conversation neutrally; ds-andreessen takes
  a hard, market-first position and defends it.
- **vs ds-capture** (productivity): capture organizes dumps; andreessen judges bets.
- **vs the founder-operating-system / c-level personas:** those balance and advise across many roles;
  ds-andreessen is a single opinionated operator with a fixed anti-sycophancy voice and a market-first thesis.
...
- [ds-reflect](../../reflect/agents/ds-reflect.md) — productivity sibling, neutral reassessment
