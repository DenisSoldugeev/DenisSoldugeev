---
name: "ds-roast"
description: "/ds:roast — Convene a 5-angle adversarial panel (Critic, Champion, Analyst, Investigator, Customer) on an idea, then a Judge delivers one GO / RESHAPE / KILL verdict with the cheapest 48-hour test to de-risk it. Pressure-test before you build."
argument-hint: "[the idea to roast]"
---

# /ds:roast — 5-Angle Idea Panel → One Verdict

**Command:** `/ds:roast [the idea]`
...
- A cross-functional enterprise decision needing the full C-suite → use `/ds:boardroom`.
...
- Agent: [`ds-roast-judge`](../agents/ds-roast-judge.md)
- Skill: [`roast`](../skills/roast/SKILL.md)
- Siblings: `/ds:andreessen` (single market-first lens), `/ds:boardroom` (enterprise C-suite)
