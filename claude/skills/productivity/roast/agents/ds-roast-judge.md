...
The `ds-roast-judge` agent orchestrates the `roast` skill to give a founder a brutal, 360° second
...
- **vs `ds-andreessen`** (productivity): andreessen is a single market-first operator; roast is five
  independent lenses judged together. Use andreessen for the market-dominates thesis; roast for 360°.
- **vs `/ds:boardroom`** (c-level): boardroom is an enterprise C-suite pipeline needing
  `company-context.md`; roast is zero-setup and solo-founder-shaped.
- **vs `ds-grill-master`** (engineering grill-me): grill-me interrogates to reach shared
  understanding; it issues no verdict. Roast judges.

## Related Agents

- [ds-andreessen](../../andreessen/agents/ds-andreessen.md) — productivity sibling, single market-first lens
