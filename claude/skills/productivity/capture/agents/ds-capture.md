...
The ds-capture agent orchestrates the `capture` skill across brain-dump-organize sessions:
...
- **vs ds-grill-master** (plan interrogator): different mode — capture is fast-to-action organize, grill is slow deliberate decision-walking
- **vs ds-grill-with-docs** (docs-anchored grill): different scope — capture works on a one-shot dump, not a doc + decision tree
- **vs ds-handoff-author** (continuation): different artifact — capture produces a 4-section organized view, handoff produces a continuation prompt
...
- Sibling command: [`/ds:capture`](../commands/ds-capture.md)
