---
name: "ds-capture"
description: "/ds:capture <dump-text-or-path> — Explicit invocation of the brain-dump organizer. Captures an unstructured stream of thoughts/tasks/ideas and returns a 4-section actionable system (Projects/Ideas, Tasks, Connections, How I Can Help). Compressed format for small dumps. Max 1 clarifying question. No fabricated connections. No corporate-ifying."
---

# /ds:capture — Brain-Dump Organizer

**Command:** `/ds:capture <dump-text-or-path>`

The `ds-capture` persona organizes a dump into 4 actionable sections with zero information loss.
...
The skill ALSO triggers automatically without `/ds:capture` when you:
...
`/ds:capture` is the explicit form — useful when your dump doesn't include the trigger phrasing but you still want the organize behavior.
...
- Agent: [`ds-capture`](../agents/ds-capture.md)
...
- Adjacent commands: `/ds:grill-me` (slow deliberate plan grill), `/ds:grill-with-docs` (docs-anchored grill), `/ds:handoff` (session continuation)
