...
The ds-skill-author agent orchestrates the `write-a-skill` skill across the three skill-authoring decisions Matt Pocock named:
...
- **vs raw write-a-skill skill** (no persona): the skill provides the workflow; ds-skill-author provides the interrogation gate before commit.
- **vs ds-tdd-guide** (testing): different concern (test code vs skill files).
- **vs ds-tc-tracker** (task context): different concern (per-task context vs reusable skill).
...
# 4. Add wrapper layer per this repo's pattern (validators + references + ds-* + /ds:*)
...
- [ds-karpathy-coder](../../karpathy-coder/agents/karpathy-reviewer.md) — Code quality gate (complexity_checker, diff_surgeon)
- [ds-tdd-guide](../../../engineering-team/skills/tdd-guide/) — Test discipline for code (not skill files)
...
- Sibling command: [`/ds:write-a-skill`](../commands/ds-write-a-skill.md)
