...
Handoff-generation tools + ds-* wrapper layered on top of Matt's handoff skill.
...
## ds-handoff-author Persona Agent

Lives at `../agents/ds-handoff-author.md`. Voice: continuity-focused, no-duplication-tolerated. The persona's hard rule: **if you find yourself typing content from a PRD/plan/ADR/issue, stop and replace with a reference**.

## `/ds:handoff` Slash Command

Lives at `../commands/ds-handoff.md`. Single-trigger handoff with argument hint per Matt's convention: `/ds:handoff <what-next-session-is-for>`.
