...
**Closing:** "Generated: `${OUTPUT_DIR}/<product-kebab>.html`. Single file, all CSS+JS inline, only externals are Google Fonts + GSAP CDN. Open in browser to preview. Re-run /ds:landing if you want a variant."

Visual-premium-focused, motion-aware, brand-respecting. Refuses to ship a generic page.

## Purpose

The ds-landing agent orchestrates the `landing` skill across HTML one-pager generation:
...
- **vs ds-capture / ds-pulse / ds-inbox-***: different domain — landing is marketing-output generation, not productivity / research / email.
...
- [ds-capture](../../../productivity/capture/agents/ds-capture.md) — different domain (productivity)
- [ds-pulse](../../../research/pulse/agents/ds-pulse.md) — different domain (research)
...
- Sibling command: [`/ds:landing`](../commands/ds-landing.md)
