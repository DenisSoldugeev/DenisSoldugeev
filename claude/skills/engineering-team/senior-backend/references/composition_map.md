...
This map is the routing table for the `ds-backend-engineer` agent and the `/ds:backend-review` command.
...
| Pre-commit Karpathy review | **ds-karpathy-reviewer** | Before EVERY commit | `../../../engineering/karpathy-coder/` |
| Pre-flight architecture grill | **ds-grill-master** | Before locking pattern or DB choice | `../../../engineering/grill-me/` |
...
- ❌ Skipping `ds-karpathy-reviewer` before commit — every commit must pass the diff-noise gate.
...
- **Frontend integration questions** → escalate to `ds-frontend-engineer`.
- **Org-design / capacity / hiring** → escalate to `ds-vpe-advisor` (engineering) or `ds-bizops-orchestrator` (cross-functional ops).
- **Strategic build-vs-buy at company level** → escalate to `ds-cto-advisor`.
- **AI/ML pipeline + model serving** → escalate to `senior-ml-engineer`.
- **Data warehouse / dbt / lakehouse** → escalate to `senior-data-engineer`.
- **Pure security threat model** → escalate to `ds-ciso-advisor` (strategic) or `senior-security` (tactical).
