...
The `ds-backend-engineer` agent orchestrates these forks via `context: fork`. Invoke it from another agent with `Agent({subagent_type: "ds-backend-engineer", prompt: "..."})` or via `/ds:backend-review <your problem>`.
...
1. **Slash command:** `/ds:backend-review <prompt>` — full grill + decision engine + composition routing.
2. **Agent subagent:** `Agent({subagent_type: "ds-backend-engineer", prompt: "..."})` — forks context, returns ≤ 200-word digest.
3. **Direct tool call:** `python scripts/backend_decision_engine.py ...` — deterministic profile match when inputs are known.

See `agents/engineering/ds-backend-engineer.md` for the full invocation contract.
