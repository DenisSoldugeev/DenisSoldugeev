...
The engine encodes the same matrix the conversational grill walks through — use it directly when inputs are already known, or via the `ds-fullstack-engineer` agent for the question-by-question grill.
...
The `ds-fullstack-engineer` agent (in `agents/engineering/ds-fullstack-engineer.md`) orchestrates these forks via `context: fork`. Invoke it from another agent with `Agent({subagent_type: "ds-fullstack-engineer", prompt: "..."})` or via the slash command `/ds:fullstack-review <your problem>`.
...
1. **Slash command:** `/ds:fullstack-review <prompt>` — runs the full grill + decision engine + composition routing.
2. **Agent subagent:** `Agent({subagent_type: "ds-fullstack-engineer", prompt: "..."})` — forks context, returns ≤ 200-word digest.
...
See `agents/engineering/ds-fullstack-engineer.md` for the full invocation contract.
