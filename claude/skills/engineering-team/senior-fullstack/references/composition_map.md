...
This map is the routing table for the `ds-fullstack-engineer` agent and the `/ds:fullstack-review` command.
...
| Pre-commit code review (Karpathy 4 principles) | **ds-karpathy-reviewer** | Before EVERY commit this skill produces | `../../../engineering/karpathy-coder/` |
| Pre-flight grill on a draft architecture | **ds-grill-master** | Before locking the stack picks | `../../../engineering/grill-me/` |
...
- ❌ Skipping `ds-karpathy-reviewer` before committing. Every commit from this skill must pass the diff-noise gate.
...
- **Pure-engineering org-design questions** (team topologies, manager triggers) → escalate to `ds-vpe-advisor`.
- **Strategic build-vs-buy at the company level** → escalate to `ds-cto-advisor`.
