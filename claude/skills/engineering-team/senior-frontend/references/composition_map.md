...
This map is the routing table for the `ds-frontend-engineer` agent and the `/ds:frontend-review` command.
...
| Pre-commit Karpathy review on changed files | **ds-karpathy-reviewer** | Before EVERY commit this skill produces | `../../../engineering/karpathy-coder/` |
| Pre-flight architecture grill | **ds-grill-master** | Before locking framework or rendering model | `../../../engineering/grill-me/` |
...
- ❌ Skipping `ds-karpathy-reviewer` before committing — every commit must pass the diff-noise gate.
- ❌ Treating shipped UI as a final product without `playwright-pro` visual-regression baseline.

## When to escalate out of frontend

- **Brand voice / copy** → escalate to `marketing-skill/content-creator` + `ds-content-creator` agent.
- **Backend API design** → escalate to `ds-backend-engineer` + `api-design-reviewer`.
- **iOS/macOS-native UI** → escalate to `ds-apple-hig` (if present) or `product-team/skills/apple-hig-expert`.
- **Marketing-site infrastructure choice (Astro vs Next vs Hugo)** → escalate to `ds-fullstack-engineer` (marketing-site profile).
